...
const init = async () => {
  try {
    ...
    while ((doBlockMatch = doBlockPattern.exec(cleanedSchema)) !== null) {
      ...
      doBlockRanges.push([
        doBlockMatch.index,
        doBlockMatch.index + doBlockMatch[0].length,
      ]);
    }
      ...
      try {
        ...
        if (
          ...
          if (statement.includes("idx_teachers_school")) {
            ...
            try {
              await pool.query(
                "ALTER TABLE teachers ADD COLUMN IF NOT EXISTS school_id INTEGER REFERENCES schools(id) ON DELETE CASCADE",
              );
              ...
              errorCount++;
              console.warn(
                `⚠ Warning in statement ${i + 1}: ${error.message.substring(0, 100)}`,
              );
            }
          } else {
            errorCount++;
    ...
    console.log(
      `✓ Database schema synchronized (${successCount} statements executed)`,
    );
    if (errorCount > 0) {
