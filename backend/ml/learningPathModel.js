...
// Generate learning path based on subject and section
const generateLearningPath = async (data) => {
  try {
    const { subject, section, studentId } = data;

    // If a trained model exists, prefer it for ranking (fast + consistent)
    const model = loadTrainedModel();
    if (model) {
      const subjectNorm = (subject || '').toString().toLowerCase().trim();
      const queryTokens = toTokens(`${subject} ${section}`);
      const { vec: qVec, norm: qNorm } = buildQueryVector(queryTokens, model.idf);

      const candidates = model.docs
        .filter(d => !subjectNorm || (d.subject || '').toString().toLowerCase().trim() === subjectNorm);

      const scored = candidates
        .map(d => ({
          id: d.id,
          score: cosineSimilarity(qVec, qNorm, d.vector || {}, d.norm || 1)
        }))
      ...
      if (scored.length > 0) {
        ...
        const resourcesRes = await db.query(
          'SELECT * FROM sinhala_resources WHERE id = ANY($1::int[])',
          [ids]
        );
        ...
        if (topResources.length > 0) {
          const pathContent = generatePathContent(topResources, subject, section);
    ...
    // Get Sinhala resources from database
    const resources = await db.query(
      `SELECT * FROM sinhala_resources 
       WHERE subject ILIKE $1 
       AND (section ILIKE $2 OR section ILIKE '%' || $2 || '%')
       ORDER BY grade_level ASC`,
      [subject, section]
    );

    // If no resources found, search more broadly
    let sinhalaResources = resources.rows;
    
    if (sinhalaResources.length === 0) {
      const broadResources = await db.query(
        `SELECT * FROM sinhala_resources 
         WHERE subject ILIKE $1 
         ORDER BY grade_level ASC
         LIMIT 10`,
      ...
      sinhalaResources = broadResources.rows;
    }

    // If still no resources, generate a template path
    if (sinhalaResources.length === 0) {
      return generateTemplatePath(subject, section);
    }

    // Use TF-IDF to rank and select best resources
    const tfidf = new TfIdf();
    const sectionKeywords = section.toLowerCase().split(/\s+/);
    
    sinhalaResources.forEach((resource, index) => {
      const text = `${resource.content} ${resource.section} ${resource.subject}`.toLowerCase();
    ...
    // Sort by score and take top resources
    scoredResources.sort((a, b) => b.score - a.score);
    const topResources = scoredResources.slice(0, 5).map(sr => sr.resource);
...
// Generate path content from resources
const generatePathContent = (resources, subject, section) => {
  let content = `\n${subject} - ${section} සඳහා ඉගෙනුම් මාර්ගය\n\n`;
  content += `මෙම ඉගෙනුම් මාර්ගය ${section} කොටස හැදෑරීමට උපකාරී වනු ඇත.\n\n`;

  resources.forEach((resource, index) => {
    content += `${index + 1}. ${resource.resource_type === 'lesson' ? 'පාඩම' : resource.resource_type === 'exercise' ? 'ව්‍යායාම' : 'උදාහරණය'}\n`;
  ...
  content += `4. නිතර පුනරීක්ෂණය කරන්න\n`;

  return content;
};

// Generate template path when no resources found
const generateTemplatePath = (subject, section) => {
  const content = `\n${subject} - ${section} සඳහා ඉගෙනුම් මාර්ගය\n\n` +
    `මෙම කොටස හැදෑරීමට පහත පියවර අනුගමනය කරන්න:\n\n` +
    `1. මූලික සංකල්ප හැදෑරීම\n` +
...
// This isn't a neural network; it's a fast retrieval model to pick the best resources for a topic.
const trainLearningPathModel = async () => {
  const res = await db.query('SELECT id, subject, section, content FROM sinhala_resources');
  const rows = res.rows || [];
  if (rows.length === 0) {
