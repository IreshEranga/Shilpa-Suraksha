...
/**
 * Commented below code on 8/03/2026 to implement dynamic thresholds based on historical data analysis.
 * This allows the model to adapt to changing student performance trends and provides more accurate identification of weak students.
 */
// let tf;
// try {
//   tf = require('@tensorflow/tfjs-node');
...
const db = require('../config/database');
const { getFreshThresholds } = require('./thresholdCalculator');
...
const trainModel = async () => {
  try {
    // Get fresh dynamic thresholds from historical data
    const thresholds = await getFreshThresholds();
    console.log('🎯 Training with fresh dynamic thresholds:');
    console.log('   Academic critical threshold:', (thresholds.academic.criticalScoreThreshold * 100).toFixed(1) + '%');
    console.log('   Attendance high risk threshold:', (thresholds.attendance.highRiskThreshold * 100).toFixed(1) + '%');
    ...
    const useRuleBasedLabeling = weakStudentIds.size === 0;
    if (useRuleBasedLabeling) {
      console.log('No labeled weak students found. Using rule-based labeling with fresh dynamic thresholds...');
    }
    ...
    for (const academic of academicData.rows) {
      ...
      if (featureArray.every(val => typeof val === 'number' && isFinite(val))) {
        ...
        let label = 0;
        if (useRuleBasedLabeling) {
          // Rule-based using fresh dynamic thresholds from historical data
          label = (
            avgScore < thresholds.academic.criticalScoreThreshold || 
            attendanceRate < thresholds.attendance.highRiskThreshold
          ) ? 1 : 0;
    ...
    console.log('✓ Academic model trained successfully with fresh dynamic thresholds');
  } catch (error) {
    console.error('Error training academic model:', error);
  }
};
...
const identifyWeakStudents = async (data) => {
  try {
    ...
    // Build reasons based on FRESH DYNAMIC THRESHOLDS
    const reasons = [];
    if (avgScore < thresholds.academic.lowScoreThreshold) {
      reasons.push(`Low average score (${(avgScore * 100).toFixed(1)}% vs threshold ${(thresholds.academic.lowScoreThreshold * 100).toFixed(1)}%)`);
    }
...
const fallbackIdentification = async (data) => {
  ...
  // Use fresh dynamic thresholds instead of hardcoded values
  const isWeak = avgScore < thresholds.academic.lowScoreThreshold || 
                 attendanceRate < thresholds.attendance.mediumRiskThreshold;
  const confidence = isWeak ? 0.8 : 0.2;
